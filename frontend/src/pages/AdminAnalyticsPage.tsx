...
export default function AdminAnalyticsPage() {
  ...
  // Updated data remapping for charts
  const renderChart = (data: any[], chartType?: string) => {
    ...
    if (chartType === "bar") {
      // Map backend keys to recharts keys for bar charts
      chartData = data.map(item => ({
        name: item.customer_id || item.name || item.label || "Unknown",
        value: item.fraud_count || item.value || item.count || 0
      }));
    } else if (chartType === "pie") {
      chartData = data.map(item => ({
        name: item.name || item.label || item.customer_id || "Unknown",
